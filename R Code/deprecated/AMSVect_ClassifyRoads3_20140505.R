...
getLinePixels <- function(con, schemaname, inputlines, inputpixels) {
  ...
  linepixels.df <- dbRunScript(con, path="SQL Code/SelectLinePixels_20140527.sql", return=TRUE, script.param=list(PIXELSIZE = res(inputpixels)[1], SCHEMANAME=schemaname))
...
add.cov.node.graph <- function(con, schemaname, graph, inputlines, other.vars=c("ppl")) {
  ...
  otherlines.df <- dbRunScript(con, path="SQL Code/SelectOtherLines_20140506.sql", return=TRUE, script.param=list(SCHEMANAME=schemaname))
