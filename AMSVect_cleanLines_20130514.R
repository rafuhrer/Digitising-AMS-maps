...
# of differences in the line attributes. Returns a SLDF  with the same attributes as the input SLDF.
	...
	nondup.df <- dbRunScript(con, path="SQL Code/SelectNonDuplicateLines_20140425.sql", script.param=list(SCHEMANAME=schemaname), return=TRUE)	
...
# Returns a SPDF with REP and one attribute column called "pid" with unique point IDs.
	...
	dbRunScript(con, path="SQL Code/SelectRealEndPoints_20140425.sql", script.param=list(SCHEMANAME=schemaname))
...
# geometries (i.e., slightly reduces precision). 
	...
	rs <- dbRunScript(con, path="SQL Code/StandardizeGeom_20140425.sql", script.param=list(SCHEMANAME=schemaname))	
