...
snapNetwork <- function(con, schemaname, inputlines, buffersize=20, breakfirst=TRUE, snapAnywhere=TRUE, partitionName=NULL, parallel=FALSE, OS="Win", cores=4) {
  ...
  if (is.null(partitionName)) {
    snapcandidates.df <- dbRunScript(con, "SQL Code/SnapLinePoints_20140425.sql", return=TRUE, script.param=list(BUFFERSIZE=buffersize, SCHEMANAME=schemaname)) 
  } else {
    # Only looks for candidates snappling lines *across* partitions!
    snapcandidates.df <- dbRunScript(con, "SQL Code/SnapLinePointsPartition_20140429.sql", return=TRUE, script.param=list(BUFFERSIZE=buffersize, SCHEMANAME=schemaname, PARTITIONNAME=partitionName)) 
  }
  ...
  if (parallel) {
	...
	snapeffcand.df <- dbRunScript(con, "SQL Code/SelectEffectiveSnap_20140425.sql", return=TRUE, script.param=list(SCHEMANAME=schemaname)) 
      ...
      for (i in 1:length(cand.lid)) {
        ...
        if (length(this.v) > 1) {
          ...
          if (length(next.lid) > 1 & length(prev.lid) > 1) {
            ...
            dist <- shortest.paths(alt.graph, v=this.v1, to=this.v2)
            if (dist < maxdetour) {
